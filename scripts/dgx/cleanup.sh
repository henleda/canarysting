...
validate_artifact_tree() {
  ...
  while IFS= read -r entry; do
    case "${entry}" in
      d:product|d:test|f:manifest.tsv|f:SHA256SUMS|f:product/engine|f:product/canaryctl|f:product/operator|f:product/envoy-adapter|f:product/dashboard-backend|f:test/cookiespike|f:test/enforcespike|f:test/dgxstackspike|f:test/correlationspike)
        ;;
      *)
  ...
  if [[ "${require_complete}" == 'yes' ]]; then
    ...
    while read -r digest relative_path extra; do
      ...
      case "${relative_path}" in
        manifest.tsv|product/engine|product/canaryctl|product/operator|product/envoy-adapter|product/dashboard-backend|test/cookiespike|test/enforcespike|test/dgxstackspike|test/correlationspike)
          ;;
