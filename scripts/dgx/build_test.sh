...
"${build_script}" --output-dir "${first}" --target engine --target cookiespike
"${build_script}" --output-dir "${second}" --target engine --target cookiespike

for relative_path in product/engine test/cookiespike manifest.tsv SHA256SUMS; do
