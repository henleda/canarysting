...
"${build_script}" --output-dir "${valid}" --target engine --target cookiespike --target correlationspike --target tracespike --target attackerexecutorspike
...
[[ "${dry_output}" == *'artifacts=5'* ]] || fail 'valid dry run reported the wrong artifact count'
