...
"${build_script}" --output-dir "${valid}" --target engine --target cookiespike --target correlationspike --target tracespike --target attackerexecutorspike --target attackerloopspike
...
[[ "${dry_output}" == *'artifacts=6'* ]] || fail 'valid dry run reported the wrong artifact count'
