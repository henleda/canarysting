...
cleanup_prelude='fail() { printf "FAIL: %s\n" "$*" >&2; exit 1; }'$'\n''stat() { case "$1:$2" in -c:%F) [[ -f "$3" && ! -L "$3" ]] && printf "regular file\n" || return 1 ;; -c:%u) id -u ;; -Lc:%d:%i) if [[ "$3" == /dev/fd/7 ]]; then command stat -f %d:%i "${root}"; elif [[ "$3" == /dev/fd/8 ]]; then if [[ -e "${quarantine_name}" ]]; then command stat -f %d:%i "${quarantine_name}"; else command stat -f %d:%i "${evidence_name}"; fi; else command stat -f %d:%i "$3"; fi ;; -Lc:%h) if [[ "$3" == /dev/fd/7 && ! -e "${root}" ]] || [[ "$3" == /dev/fd/8 && ! -e "${quarantine_name}" ]]; then printf "0\n"; else command stat -f %l "$3"; fi ;; *) command stat "$@" ;; esac; }'$'\n''cd() { if [[ "${@: -1}" == /dev/fd/7/. ]]; then builtin cd -P -- "${root}"; elif [[ "${@: -1}" == /dev/fd/8/. ]]; then if [[ -e "${quarantine_name}" ]]; then builtin cd -P -- "${quarantine_name}"; else builtin cd -P -- "${evidence_name}"; fi; else builtin cd "$@"; fi; }'
...
if [[ "$(uname -s)" == 'Darwin' ]]; then
  command -v zsh >/dev/null 2>&1 || fail 'zsh is required for the macOS cleanup portability test'
...
grep -Fq 'cd -P -- /dev/fd/7/.' "${subject}" || fail 'manual cleanup does not operate through the validated root descriptor'
grep -Fq 'validated evidence root remains linked after cleanup' "${subject}" || fail 'manual cleanup does not verify unlink of the validated root inode'
...
grep -Fq 'cd -P -- /proc/self/fd/7/.' "${subject}" || fail 'expiry cleanup does not operate through the validated root descriptor'
grep -Fq 'validated evidence root remains linked after expiry' "${subject}" || fail 'expiry cleanup does not verify unlink of the validated root inode'
grep -Fq 'if [[ ! -e "${root}" && ! -L "${root}" ]]; then' "${subject}" || fail 'cleanup does not treat an already-absent evidence root as success'
...
cleanup_call_line="$(grep -n 'evidence_state="$(cleanup_evidence)"' "${subject}" | cut -d: -f1)"
cancel_call_line="$(grep -n 'cancel_expiry_timer || fail' "${subject}" | cut -d: -f1)"
[[ "${cleanup_call_line}" -lt "${cancel_call_line}" ]] || fail 'manual cleanup cancels the retry timer before evidence deletion succeeds'
