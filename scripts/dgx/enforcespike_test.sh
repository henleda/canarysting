...
cleanup_prelude='fail() { printf "FAIL: %s\n" "$*" >&2; exit 1; }'$'\n''stat() { case "$1:$2" in -c:%F) [[ -f "$3" && ! -L "$3" ]] && printf "regular file\n" || return 1 ;; -c:%u) id -u ;; -Lc:%d:%i) if [[ "$3" == /dev/fd/8 ]]; then command stat -f %d:%i "${evidence_name}"; else command stat -f %d:%i "$3"; fi ;; *) command stat "$@" ;; esac; }'$'\n''cd() { if [[ "${@: -1}" == /dev/fd/8/. ]]; then builtin cd -P -- "${evidence_name}"; else builtin cd "$@"; fi; }'
...
cleanup_state="$(zsh -c "${cleanup_runner}" -- "${cleanup_run_id}" "${cleanup_root}")" || fail 'anchored cleanup rejected oversized failed-run evidence'
...
CDPATH="${cdpath_external}" zsh -c "${cleanup_runner}" -- "${cdpath_run_id}" "${cleanup_root}" >/dev/null || fail 'anchored cleanup honored inherited CDPATH'
