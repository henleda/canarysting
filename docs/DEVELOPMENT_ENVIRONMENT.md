...
- `enforcespike.sh` — the M1D precise containment/Cilium coexistence proof. It accepts only a strict run ID and fixed run/inspect/cleanup modes, re-verifies the checksum-bound `test/enforcespike` stage, snapshots that artifact into a root-owned mode-0700 directory, and executes it under a fixed timeout in `/sys/fs/cgroup/canarysting-dgx/<run-id>`. The binary attaches the production sockops, cgroup-skb egress, and sock-release programs only to that child; creates independent target and control loopback sockets; validates both socket cookies against `SO_COOKIE`; proves both flows before enforcement; treats only the target as an explicit proof-fixture canary touch; programs only that cookie through `containment.Apply`; proves target drops and kernel counters while the control remains a verdict-map miss; releases the target and verifies that the same connection recovers. The harness observes all three programs live on the child and absent from parent/root, compares root attachments plus complete BPF program/map/link inventories before/after, bounds each evidence file to 1 MiB, and refuses pre-existing named CanarySting BPF state. Runtime cleanup removes only its exact empty child cgroup and privileged snapshot; `--cleanup` additionally validates/removes the exact evidence and delegates the verified artifact stage to the generic cleanup workflow. It never changes Cilium or Kubernetes state.
