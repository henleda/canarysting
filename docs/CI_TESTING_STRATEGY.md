...
PR concurrency is keyed by PR number with cancellation enabled, and checkout is pinned to the latest PR head SHA. Remote work depends on a green local Level 2 job. `scripts/dgx/pr.sh` accepts only fixed profiles and run IDs. A compatible scenario workflow performs one `check.sh` preflight, one allowlisted ARM64 build, and one checksum-verified transfer. The read-only `attacker-check` profile performs the preflight and exact readiness inspection without a build or transfer. A short-lived run/revision/host-bound proof lets compatible leaf scenarios reuse that inspection; leaf stage verification, before/after assertions, platform health checks, safety stops, and cleanup remain mandatory.
