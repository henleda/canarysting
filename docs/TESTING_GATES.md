...
| DGX scripts | Relevant local DGX harness contracts plus CRITICAL remote profile selection; trace, attacker readiness, executor, loop, and reproducible-scenario paths select their exact profiles |
...
The local scenarios are deterministic fixture proofs, not a claim that the complete CanaryAttacker/Qwen correlation laboratory exists. M2C.3 and M2C.4 add exact loopback DGX smokes. M2C.5 adds a real, dedicated Kubernetes target and two repeated five-step ground-truth runs, but it still does not manufacture or claim independent CanaryView evidence correlation. Observation ingestion, joins, and metrics remain M2D work and cannot be inferred from this gate.
...
Pushes to `main`, nightly schedules, and integration dispatches run Level 3 once. Weekly and campaign dispatches proceed to Level 4 only after Level 3 succeeds. The live campaign step is scheduled but deliberately fails closed until the M2D correlation/campaign contract exists; the bounded M2C.4 loopback and M2C.5 targeted Kubernetes smokes are not substituted. The privileged `ebpf-privileged` check retains its structured zero-skip PASS floor when selected.
