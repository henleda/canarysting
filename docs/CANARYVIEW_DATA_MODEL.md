...
`internal/dashboard/backend/views.ProjectTrace` converts one validated immutable trace into a bounded, read-only `TraceWorkspace` contract. It presents the ordered source-record journey, affected identity references, explicit correlation confidence, every candidate join and citation, missing declared coverage, conflicts, opaque evidence references, raw-reference availability, and trace lifecycle/model-use state. It derives only deterministic display summaries from the canonical trace; it does not create a `SecurityCase`, persist a second trace, reinterpret a source decision, or infer an action.

The dashboard backend exposes `GET /api/traces/{trace_id}` only through an injected scoped `TraceSource`. Tenant or scope selectors are not accepted from the route, a mismatched returned record is refused, and the composition root leaves the source unwired until an authenticated CanaryView query service exists. The initial Next.js workspace therefore has an explicit synthetic fixture path for deterministic operator and Playwright validation rather than pretending that production trace storage or authentication is complete. Explanation is visible without interaction; the opaque source-owned raw reference, availability, integrity, lifecycle, residency, hold, and separate model-use state are one click away. No source payload is carried by the projection.
