...
Status: approved conceptual specification (M2A.0.4, 2026-09-01) with the M2A.1 package/reuse boundary and M2A.2 minimum schema-v2 implementation recorded. Persistence technology and runtime integration remain deferred.
...
M2A.3 remains responsible for the broader assertion modes, producer classification, confidence components, verification evidence, typed missing/conflicting-evidence semantics, and acyclic transformation provenance. M2A.2 deliberately exposes no free-form parser-warning or missing-field strings; partiality is represented only by absent optional source fields until M2A.3 supplies bounded typed semantics. M2A.4 remains responsible for cross-scope query/store behavior, collection bounds, deletion/invalidation execution fixtures, and synthetic production-path rejection. M2B introduces collectors and source adapters. No current code persists an `Observation` or changes CanarySting runtime behavior.
