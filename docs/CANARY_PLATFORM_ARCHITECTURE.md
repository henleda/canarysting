...
The 2026-09-01 M2A.1 review accepts `internal/canaryview/model` as the Go source of truth for the minimum vendor-neutral CanaryView domain model. M2A.2 created only the approved schema-v2 envelope, source-report observation, evidence/opaque raw references, source/control/scope identities, and lifecycle/model-use values. Unsafe pre-consumer schema v1 is refused because its missing production/synthetic classification, source-report basis, and retention decision/clock cannot be reconstructed safely. Those fields and default Lean/Standard final expiry fail closed in v2; free-form diagnostic content is deferred to later bounded typed contracts. The model package imports only the Go standard library. It must not import CanarySting engine, contract, intelligence, adapter, dashboard, Kubernetes/vendor SDK, transport, or persistence packages.
...
The schema-v3 model also makes knowledge state, assertion mode, producer class, named confidence components, typed missing/conflicting evidence, verification procedure/evidence, and acyclic transformation lineage mandatory. Schema v1 and v2 inputs are not upgraded by inventing these fields; trusted-source re-ingestion is required. A syntactically parsed entity identifier defaults to `DECLARED`, and `VERIFIED` requires a named deterministic or operator procedure with supporting evidence. This is a neutral CanaryView contract only: live mesh/Kubernetes identity binding remains M3.2 and retains its DGX validation tier.
