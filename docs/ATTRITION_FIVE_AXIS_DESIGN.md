...
- **AX0 alone changes the live Tier-2 WIRE output (conscious-deploy gate — surfaced by the AX0 review).** At `FloorModerate`+`TierContain` — the DEPLOYED path (`canarysting-adapter.service` runs `-sting-floor 1`; the live prober / M9 LLM hits Tier 2) — the pre-AX0 stream emitted PURE `fake_tree` maze pages (the retired `selectGenerator` returned only `fakeMaze`). Under AX0 `selectAxes` returns `{tarpit, fakeMaze}` (both `minTier=TierContain`) and the stream rotates per chunk, so ~half the bytes the attacker sees become tarpit ASCII filler (velocity) interleaved with maze (poison) — the INTENDED five-axis composition (Moderate = velocity + information poisoning), NOT a regression: the headline `Outcome.Mechanism` stays `fake_tree`, so the D3 by-mechanism KPI, recon-feed grouping, and dashboard are unchanged. But it IS a visible live-demo-posture change that saved screenshots key on. Treat the on-box AX0 adapter redeploy as a conscious, WITH-Daniel step (like D8). If preserving the exact pre-AX0 live output until AX2 is preferred, construct `FloorModerate` as `fakeMaze`-only until velocity is consciously enabled — but documentation + signoff is the lighter route. **(Local build/tests are unaffected; this gates only the on-box deploy.)**
