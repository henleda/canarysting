...
| Canonical CanaryView stores — future CanaryView Core / Shared platform | M2A.4 adds a bounded in-memory production-observation proof seam; no durable normalized-observation store, raw-reference registry, `SecurityCase` store, append-only relationship journal, legal-hold service, model-use registry, feature/model registry, or production graph backend exists. | The proof seam filters ordinary reads by immutable lifecycle state/time, rejects synthetic production input, and exposes a same-scope direct-lineage invalidation frontier. It performs no physical deletion, retry, backup/index/cache cleanup, or projection rebuild. | `internal/canaryview/store`; its lifecycle, no-bleed, bound, model-use, deterministic-fixture, and race tests. No protobuf or persistent repository definition exists for `SecurityCase` or `RelationshipEvent`. | Reusable backend-neutral acceptance behavior, not production persistence. Backend choice and lifecycle execution remain planned and must prove the objectives in this document. |
