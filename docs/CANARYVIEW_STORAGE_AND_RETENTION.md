...
| Canonical CanaryView stores — future CanaryView Core / Shared platform | M2A.4 adds a bounded in-memory production-observation proof seam and M2B.4 adds a separate bounded in-memory `CORRELATED_TRACE` projection seam; no durable normalized-observation/trace store, raw-reference registry, `SecurityCase` store, append-only relationship journal, legal-hold service, model-use registry, feature/model registry, or production graph backend exists. | Both proof seams filter ordinary reads by immutable lifecycle state/time and reject synthetic production input. The trace seam enforces 90-day Lean and 13-calendar-month Standard close-based retention, keeps exact holds visible, and records exact-scope direct-lineage invalidation overlays for parent expiry/deletion/invalidation. Neither performs physical deletion, retry, backup/index/cache cleanup, recursive invalidation, or projection rebuild. | `internal/canaryview/store` and `internal/canaryview/trace`; lifecycle, no-bleed, bound, model-use, deterministic-fixture, ambiguity, broken-reference, invalidation, and race tests. No external trace serialization or persistent repository definition exists for `SecurityCase` or `RelationshipEvent`. | Reusable backend-neutral acceptance behavior, not production persistence. Backend choice and lifecycle execution remain planned and must prove the objectives in this document. |
