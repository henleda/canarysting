...
Status: approved architecture baseline (M2A.0.4, 2026-09-01), with the M2C.2 repository-backed synthetic ground-truth corpus mapping recorded. This document defines lifecycle principles and logical storage requirements. It does not select or implement a production database, change current CanarySting runtime behavior, or authorize collection of additional data.
...
| Canonical CanaryView stores — future CanaryView Core / Shared platform | M2A.4 adds a bounded in-memory production-observation proof seam and M2B.4 adds a separate bounded in-memory `CORRELATED_TRACE` projection seam; no durable normalized-observation/trace store, raw-reference registry, `SecurityCase` store, append-only relationship journal, legal-hold service, model-use registry, feature/model registry, or production graph backend exists. | Both production proof seams filter ordinary reads by immutable lifecycle state/time and reject synthetic input. The trace package also exposes a distinct in-memory laboratory namespace that accepts only synthetic traces carrying its configured scenario ID; it does not weaken the production-store rejection. The trace seam enforces 90-day Lean and 13-calendar-month Standard close-based retention, keeps exact holds visible, and records exact-scope direct-lineage invalidation overlays for parent expiry/deletion/invalidation. Neither performs physical deletion, retry, backup/index/cache cleanup, recursive invalidation, or projection rebuild. | `internal/canaryview/store` and `internal/canaryview/trace`; lifecycle, no-bleed, bound, model-use, deterministic-fixture, ambiguity, broken-reference, synthetic-namespace, invalidation, and race tests. No external trace serialization or persistent repository definition exists for `SecurityCase` or `RelationshipEvent`. | Reusable backend-neutral acceptance behavior, not production persistence. Backend choice and lifecycle execution remain planned and must prove the objectives in this document. |
| CanaryAttacker schema-v1 corpus — `internal/canaryattacker/groundtruth` | Internal `SYNTHETIC_GROUND_TRUTH`: one versioned scenario plus fixed-seed intent/action records, with exact lab tenant/scope/deployment/residency/key/registry boundaries and direct lineage. No raw payload, locator, credential, secret, customer data, or trusted network observation is present. | The reviewable repository golden fixture is the durable source. Records serialize independently so intent can be persisted before action, then bundle into the same canonical replay corpus. They explicitly use indefinite versioned-lab retention (`expires_at=null`) with a mandatory future corpus-review date; explicit corpus/version deletion removes the fixture and any lab copies. Production model use is disabled for both per-tenant and cross-tenant paths. The package does not implement a backend, legal-hold service, physical deletion worker, backup, or runtime collector. | Canonical strict JSON and constructor tests cover lineage, scope/run/parent isolation, synthetic classification, redaction, semantic identity, aggregate and per-record resource bounds, a 32-MiB pre-decode limit, copy safety, and success/failure/denial. Raw corpus ingestion and an explicit synthetic schema-v3 projection are both rejected by production observation paths. | Estimated per-record storage is mandatory and labeled assumed/measured/benchmarked. The canonical M2C.2 fixture assumes 8,192 bytes for its scenario, 4,096 bytes per intent, 4,096 bytes per attempted action, and 2,048 bytes for a denial; later measured corpus generation must update the basis rather than silently treating these assumptions as capacity evidence. |

M2B.5 adds no persistent data class. `TraceWorkspace` is a rebuildable, read-only presentation projection of an existing `CORRELATED_TRACE`; it carries the trace's lifecycle and model-use facts and exposes only opaque, source-owned raw-evidence references. Its operator fixture is synthetic and laboratory-only, and the dashboard backend deliberately has no concrete trace source until an authenticated, exact-scope query service is available. It stores no payload, case, action, trace history, or raw evidence.
...
The M2A.2 normalized-observation contract records that choice as `OBSERVED_TIME` or `INGEST_TIME_FALLBACK`, requires a retention-decision reference, and refuses a record whose `retention_start` does not equal the selected trusted timestamp. Its accepted default final expiries are six calendar months for Lean and 13 calendar months for Standard; a different period requires the existing explicit Regulated or approved-override policy path. Hot-tier transitions remain a later storage-service concern and do not rewrite the immutable final expiry.
