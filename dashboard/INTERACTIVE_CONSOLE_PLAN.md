...
### 6.1 Five-axis attrition reframe — forward presentation note (2026-06-10)

**Forward-looking, not a console rebuild.** The attrition spec was reframed from "tarpit + token-burning" to a **five-axis model** (velocity disruption / information poisoning [core differentiator] / opportunity-cost injection / exploit-inventory burn / operational exposure) with an **engagement contest** (time-to-disengage as a core metric) and a framing shift from economic-$ to **"opportunity cost on a velocity-dependent adversary."** Authoritative spec + build plan: `docs/ATTRITION_FIVE_AXIS_DESIGN.md` (the attrition build is namespaced as its own spine/axis milestones — NOT this console's M-numbers). When that build lands, the cost surfaces this console renders change as follows; record here so the drill-down `/cost` page and the `CostView`/`AttackerCost` components are updated in lockstep when the new wire fields ship:

- **Attacker-cost panel reframes from economic-$ to opportunity cost.** Lead with **time imposed + engagement** (median / longest time-to-disengage); **demote** tokens/compute/$ to a qualified **PROXY/ESTIMATE** below the imposed-time number; keep the M9 `RealMeter` ($, the only ground-truth figure) but **subordinated and NEVER merged** with the proxy. The panel tag moves from `attrition · economics` → `attrition · opportunity cost`.
- **Per-axis subtotals are INTENTIONALLY OVERLAPPING bars** — a flow's mechanism lands on multiple axes, so the per-axis set is "axes active," **never a partition summing to the flat total.** Do not render it as a 100%-stacked bar.
- **New engagement / time-to-disengage stat** on the cost surfaces — sourced from `Sting.TimeHeldSec` + the persisted disengage classifier (`DisengageReason`), **not** `LastSeen−FirstSeen` (that stays a separately-labelled "session span / persistence" metric). Defender-stops are never mislabeled as an attacker disengage.
- **By-mechanism rollup gains the new `Mech*` labels.** `DeriveCostBreakdown` groups by the raw `Sting.Mechanism` string with no hard-coded set (gap #6 already covers this), so new labels (`poison_field`, plus `MechExploitBait`/`MechOpExposure` when axes 4/5 ship) surface automatically — but the **D8 live-window flip** of the Tier-2 demo mechanism `fake_tree` → `poison_field` is a visible behavioral change needing founder signoff (recon-feed / by-mechanism rollup / saved screenshots key on `fake_tree`).
- **New wire fields are additive (gob/proto-forward-safe):** the attrition spine adds `Axes`, `TimeToDisengageSec`, `PoisonClass`, `PoisonReached`, `ExploitsObserved`, `ExposureSignals`, `DisengageReason` to `StingOutcome`; this console's `TouchEvent`/`FlowCost`/`MechanismCost` shapes gain the corresponding per-axis/engagement fields then (mirror the Go view structs 1:1 in `types.ts`, as today). Until those fields ship, the current single-axis cost rendering stays honest. See `docs/ATTRITION_FIVE_AXIS_DESIGN.md` §8 (engagement contest + dashboard reframe) for the authoritative version.
