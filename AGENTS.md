...
- `internal/canaryattacker/groundtruth/` — **implemented contract leaf:** the standard-library-only schema-v1 synthetic-lab contract defines versioned bounded scenarios, stable steps, opaque targets/fixtures, independently serializable pre-action intent and post-action success/failure/denial truth, exact scope/run lineage, and deterministic strict fixed-seed corpus JSON. It has no model/network authority, production ingestion path, or selected persistence backend.
- `internal/canaryattacker/executor/` — **implemented bounded execution seam:** an immutable exact-target/fixture/operation policy exposes only seven reviewed HTTP/DNS/TCP/enumeration/link/fixture-credential/response tools. It commits intent before resolution/dial, disables ambient proxies, pins private-lab addresses, rejects redirects/address-set changes, enforces external budgets, audits denials/failures/cancellation, and has no shell/filesystem/container/Kubernetes/control-plane authority. The bounded Qwen loop and reproducible scenario runner consume this seam.
- `internal/canaryattacker/scenarios/` — **implemented initial journey:** the fixed M2C.5 five-step enumeration, HTTP-probe, disposable fixture-credential, canary-discovery, and same-target canary-touch journey binds the closed executor to one runtime private address while keeping address and credential material out of scenario ground truth. Its DGX profile runs twice against one dedicated, run-labeled Kubernetes fixture and compares stable semantics before exact cleanup. This is synthetic ground truth, not independent observation or campaign evidence.
