...
- `internal/canaryview/model/` — **minimum implementation:** the standard-library-only CanaryView domain leaf now defines the immutable schema-v2 envelope, explicitly source-report observation, evidence/opaque raw references, source/control/scope identities, explicit production/synthetic classification, and lifecycle/model-use values with trusted retention clocks. Unsafe pre-consumer schema v1 is rejected and requires trusted-source re-ingestion. Correlation confidence/provenance, typed diagnostic semantics, stores, collectors, graph/cases, and external protobuf transport remain later tasks; CanarySting core runtime packages do not import it.
