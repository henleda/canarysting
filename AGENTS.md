...
- `internal/canaryview/model/` — **minimum implementation:** the standard-library-only CanaryView domain leaf now defines the immutable schema-v1 envelope, observation, evidence/raw references, source/control/scope identities, and lifecycle/model-use values. Correlation confidence/provenance, stores, collectors, graph/cases, and external protobuf transport remain later tasks; CanarySting runtime packages do not import it.
